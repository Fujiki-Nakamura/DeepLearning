'''
from https://gist.github.com/Kaixhin/57901e91e5c5a8bac3eb0cbbdd3aba81
'''
...
class LSTMCell(nn.LSTMCell):
    def forward(self, input, hx):
        ...
        # Weights combined into one matrix
        ...
        f = F.sigmoid(wxh[:, self.hidden_size:2 *
                          self.hidden_size])  # Forget gate
        g = F.tanh(wxh[:, 2 * self.hidden_size:3 *
                       self.hidden_size])  # Cell gate?
...
class PeepholeLSTMCell(nn.LSTMCell):
    ...
    def forward(self, input, hx):
        ...
        wxhc = wx + wh + torch.cat((wc[:, :2 * self.hidden_size], Variable(
            self.wc_blank).expand_as(h), wc[:, 2 * self.hidden_size:]), 1)
